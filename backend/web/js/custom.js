function readURL1(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            $('#upload_make_path_admin_holder').attr('src', e.target.result);
        };
        reader.readAsDataURL(input.files[0]);
    }
}

$("#upload_make_path_admin").change(function () {
    $('#upload_make_path_admin_holder').show("");
    readURL1(this);
});
...
function readURL(input, image_div_id, populate_div_id) {
    if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = function (e) {
            $('#main_image_container_src').attr('src', e.target.result);
        };
        reader.readAsDataURL(input.files[0]);
    }
}
$("#select_main_images_input").change(function () {
    ...
    readURL(this);
    $('#select_image_featured_image_text_span').text("Change Main Image");
    $('#main_image_icon').hide("");
    $('#main_image_container_src').show("");
});
